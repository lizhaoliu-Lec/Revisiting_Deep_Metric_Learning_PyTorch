...
def single_table(vals):
    ...
    for name, metrics in vals.items():
        ...
        add = '{0} & ${1:2.2f}\\pm{2:2.2f}$ & ${3:2.2f}\\pm{4:2.2f}$ & ${5:2.2f}\\pm{6:2.2f}$ & ${7:2.2f}\\pm{8:2.2f}$ & ${9:2.2f}\\pm{10:2.2f}$ & ${11:2.2f}\\pm{12:2.2f}$'.format(
            name,
...
def shared_table():
    ...
    cub_vals_2 = {name_adjust(n, for_plot=False, prep='R-' if 'reg_' in n else ''): item for n, item in
                  cub_vals.items()}
    car_names = [name_adjust(n, for_plot=False, prep='R-' if 'reg_' in n else '') for n in car_names]
    car_vals_2 = {name_adjust(n, for_plot=False, prep='R-' if 'reg_' in n else ''): item for n, item in
                  car_vals.items()}
    sop_names = [name_adjust(n, for_plot=False, prep='R-' if 'reg_' in n else '') for n in sop_names]
    sop_vals_2 = {name_adjust(n, for_plot=False, prep='R-' if 'reg_' in n else ''): item for n, item in
                  sop_vals.items()}
    ...
    for name in unique_names:
        cub_rm, cub_rs = ('{0:2.2f}'.format(cub_vals_2[name]['R@1'][0] * 100),
                          '{0:2.2f}'.format(cub_vals_2[name]['R@1'][1] * 100)) if name in cub_vals_2 else ('-', '-')
        cub_nm, cub_ns = ('{0:2.2f}'.format(cub_vals_2[name]['NMI'][0] * 100),
                          '{0:2.2f}'.format(cub_vals_2[name]['NMI'][1] * 100)) if name in cub_vals_2 else ('-', '-')
        car_rm, car_rs = ('{0:2.2f}'.format(car_vals_2[name]['R@1'][0] * 100),
                          '{0:2.2f}'.format(car_vals_2[name]['R@1'][1] * 100)) if name in car_vals_2 else ('-', '-')
        car_nm, car_ns = ('{0:2.2f}'.format(car_vals_2[name]['NMI'][0] * 100),
                          '{0:2.2f}'.format(car_vals_2[name]['NMI'][1] * 100)) if name in car_vals_2 else ('-', '-')
        sop_rm, sop_rs = ('{0:2.2f}'.format(sop_vals_2[name]['R@1'][0] * 100),
                          '{0:2.2f}'.format(sop_vals_2[name]['R@1'][1] * 100)) if name in sop_vals_2 else ('-', '-')
        sop_nm, sop_ns = ('{0:2.2f}'.format(sop_vals_2[name]['NMI'][0] * 100),
                          '{0:2.2f}'.format(sop_vals_2[name]['NMI'][1] * 100)) if name in sop_vals_2 else ('-', '-')

        add = '{0} & ${1}\\pm{2}$ & ${3}\\pm{4}$ & ${5}\\pm{6}$ & ${7}\\pm{8}$ & ${9}\\pm{10}$ & ${11}\\pm{12}$'.format(
            name,
            cub_rm, cub_rs,
            cub_nm, cub_ns,
            car_rm, car_rs,
...
def full_rel_plot():
    ...
    for k, (ax, recalls, rerrs, names, vals, ylim) in enumerate(zip(axes, recallss, rerrss, namess, valss, ylims)):
        ...
        for i in range(len(colors)):
            ax[0].errorbar(intra[intra_sort][i], recalls[intra_sort][i], yerr=rerrs[intra_sort][i], fmt='o',
                           color=colors[intra_sort][i], ecolor='gray', elinewidth=3, capsize=0, label='Basic Criteria',
                           markersize=8)
            ax[1].errorbar(inter[inter_sort][i], recalls[inter_sort][i], yerr=rerrs[inter_sort][i], fmt='o',
                           color=colors[inter_sort][i], ecolor='gray', elinewidth=3, capsize=0, label='Basic Criteria',
                           markersize=8)
            ax[2].errorbar(ratio[ratio_sort][i], recalls[ratio_sort][i], yerr=rerrs[ratio_sort][i], fmt='o',
                           color=colors[ratio_sort][i], ecolor='gray', elinewidth=3, capsize=0, label='Basic Criteria',
                           markersize=8)
            ax[3].errorbar(rho[rho_sort][i], recalls[rho_sort][i], yerr=rerrs[rho_sort][i], fmt='o',
                           color=colors[rho_sort][i], ecolor='gray', elinewidth=3, capsize=0, label='Basic Criteria',
                           markersize=8)
        ax[1].set_yticks([])
        ax[2].set_yticks([])
        ax[3].set_yticks([])
        ...
        ax[0].text(intra_lim[1] - 0.7 * (intra_lim[1] - intra_lim[0]), ylim[0] + 0.05 * (ylim[1] - ylim[0]),
                   'Corr: {0:1.2f}'.format(intra_corr), bbox=dict(facecolor='gray', alpha=0.5), fontsize=26)
        ax[1].text(inter_lim[1] - 0.7 * (inter_lim[1] - inter_lim[0]), ylim[0] + 0.05 * (ylim[1] - ylim[0]),
                   'Corr: {0:1.2f}'.format(inter_corr), bbox=dict(facecolor='gray', alpha=0.5), fontsize=26)
        ax[2].text(ratio_lim[1] - 0.7 * (ratio_lim[1] - ratio_lim[0]), ylim[0] + 0.05 * (ylim[1] - ylim[0]),
                   'Corr: {0:1.2f}'.format(ratio_corr), bbox=dict(facecolor='gray', alpha=0.5), fontsize=26)
        ax[3].text(rho_lim[1] - 0.7 * (rho_lim[1] - rho_lim[0]), ylim[0] + 0.05 * (ylim[1] - ylim[0]),
                   'Corr: {0:1.2f}'.format(rho_corr), bbox=dict(facecolor='red', alpha=0.5), fontsize=26)
...
####
def plot(vals, recalls, errs, names, reg_vals=None, reg_recalls=None, reg_errs=None, reg_names=None, xlab=None,
         ylab=None, xlim=[0, 1], ylim=[0, 1], savename=None):
    ...
    print(nnames)
    ax.errorbar(rho, recalls, yerr=errs, color='deepskyblue', fmt='o', ecolor='deepskyblue', elinewidth=5, capsize=0,
                markersize=16, mec='k')
    ...
    if reg_names is not None:
        ...
        ax.errorbar(rho, reg_recalls, yerr=reg_errs, color='orange', fmt='o', ecolor='gray', elinewidth=5, capsize=0,
                    markersize=16, mec='k')
...
plot(valss[0], recallss[0], errss[0], namess[0], reg_valss[0], reg_recallss[0], reg_errss[0], reg_namess[0],
     xlab=r'$\rho(\Phi)$', ylab=r'$CUB200-2011, R@1$', xlim=[0, 0.59], ylim=[0.58, 0.66],
     savename='Detailed_Rel_Recall_Rho_CUB')
plot(valss[1], recallss[1], errss[1], namess[1], reg_valss[1], reg_recallss[1], reg_errss[1], reg_namess[1],
     xlab=r'$\rho(\Phi)$', ylab=r'$CARS196, R@1$', xlim=[0, 0.59], ylim=[0.7, 0.84],
     savename='Detailed_Rel_Recall_Rho_CAR')
plot(valss[2], recallss[2], errss[2], namess[2], reg_valss[2], reg_recallss[2], reg_errss[2], reg_namess[2],
     xlab=r'$\rho(\Phi)$', ylab=r'$SOP, R@1$', xlim=[0, 0.59], ylim=[0.67, 0.81],
     savename='Detailed_Rel_Recall_Rho_SOP')
