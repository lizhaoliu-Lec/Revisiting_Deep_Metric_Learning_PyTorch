...
class Criterion(torch.nn.Module):
    def __init__(self, opt):
        ...
        for direction in self.directions:
            self.regressors[direction] = torch.nn.Sequential(torch.nn.Linear(self.embed_dim, self.proj_dim),
                                                             torch.nn.ReLU(),
                                                             torch.nn.Linear(self.proj_dim, self.embed_dim)).to(
                torch.float).to(opt.device)
    ...
    def forward(self, feature_dict):
        ...
        adj_feature_dict = {key: torch.nn.functional.normalize(grad_reverse(features), dim=-1) for key, features in
                            feature_dict.items()}
        ...
        for weight, direction in zip(self.weights, self.directions):
            ...
            sim_loss += -1. * weight * torch.mean(torch.mean((adj_feature_dict[target] * torch.nn.functional.normalize(
