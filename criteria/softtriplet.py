...
class Criterion(torch.nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.reg_indices = torch.zeros((self.n_classes * self.n_centroids, self.n_classes * self.n_centroids),
                                       dtype=torch.bool).to(opt.device)
    ...
    def forward(self, batch, labels, **kwargs):
        ...
        centroid_classification_loss = torch.nn.CrossEntropyLoss()(self.lam * (per_class_embed - margin_delta),
        ...
        regularisation_loss = torch.sum(
            torch.sqrt(2.00001 - 2 * inter_centroid_similarity[self.reg_indices])) / self.reg_norm
