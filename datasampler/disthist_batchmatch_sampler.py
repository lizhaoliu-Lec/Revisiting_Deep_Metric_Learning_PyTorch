...
class Sampler(torch.utils.data.sampler.Sampler):
    ...
    def precompute_indices(self):
        ...
        self.epoch_indices = Parallel(n_jobs=self.n_jobs)(
            delayed(self.disthist_match)(n_calls, i) for i in range(self.n_jobs))
    ...
    def disthist_match(self, calls, pos):
        ...
        for _ in range(calls):
            ...
            for _ in range(self.num_batch_comps):
                subset_idxs = [
                    np.random.choice(bigb_dict[np.random.choice(list(bigb_dict.keys()))], self.samples_per_class,
                ...
                dist_wd = wasserstein_distance(bigb_disthist, subset_disthist) + wasserstein_distance(subset_disthist,
                                                                                                      bigb_disthist)
