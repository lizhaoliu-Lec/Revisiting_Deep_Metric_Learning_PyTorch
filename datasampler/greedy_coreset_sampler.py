        ...
        def batchfinder(n_calls, pos):
            ...
            for i in range(len(structured_batches)):
                ...
                for class_idx in class_idxs:
                    structured_batches[i].extend(
                        [random.choice(self.image_dict[class_idx])[-1] for _ in range(self.samples_per_class - 1)])
        ...
        for _ in range(calls):
            ...
            while k < batch_size:
                if k == 0:
                    ...
                    if self.softened:
                        no = np.random.choice(
                            np.where(coreset_anchor_dists >= np.percentile(coreset_anchor_dists, 97))[0])
                    else:
