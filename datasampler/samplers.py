...
def sampler_parse_args(parser):
    parser.add_argument('--batch_selection', default='class_random', type=str,
                        help='Selection of the data batch: Modes of Selection: random, greedy_coreset')
    parser.add_argument('--primary_subset_perc', default=0.1, type=float,
                        help='Size of the randomly selected subset before application of coreset selection.')
    return parser
        ...
        for _ in range(self.sampler_length):
            ...
            if self.method == 'greedy_class_coreset':
                for _ in range(self.batch_size // self.samples_per_class):
                    ...
                    subset.extend(
                        class_indices[self.greedy_coreset(self.storage[class_indices], self.samples_per_class)])
                ...
                for _ in range(self.batch_size // self.samples_per_class):
                    class_key = random.choice(list(self.image_dict.keys()))
                    subset.extend(
                        [random.choice(self.image_dict[class_key])[-1] for _ in range(self.samples_per_class)])
