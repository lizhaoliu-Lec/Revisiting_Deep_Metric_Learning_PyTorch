...
def get_dataset(opt, data_path):
    ...
    image_classes = sorted([x for x in os.listdir(image_source_path) if '._' not in x],
                           key=lambda x: int(x.split('.')[0]))
    label_to_class_name = {int(x.split('.')[0]) - 1: x.split('.')[-1] for x in image_classes}
    label_to_image_path = {int(key.split('.')[0]) - 1: sorted(
        [image_source_path + '/' + key + '/' + x for x in os.listdir(image_source_path + '/' + key) if '._' not in x])
        for key in image_classes}
    label_to_image_path = [[(key, img_path) for img_path in label_to_image_path[key]]
                           for key in label_to_image_path.keys()]
    label_to_image_path = [x for y in label_to_image_path for x in y]

    # Dictionary of structure class:list_of_samples_with_said_class
    image_dict = {}
    for key, img_path in label_to_image_path:
    ...
    if opt.use_tv_split:
        ...
        val_conversion = {i: label_to_class_name[key] for i, key in enumerate(val)}
    ...
    train_conversion = {i: label_to_class_name[key] for i, key in enumerate(train)}
    test_conversion = {i: label_to_class_name[key] for i, key in enumerate(test)}
